def pivotAttributes(raw_df, VERBOSE=False, outputExcel=False):
    ...
    mapNames = {'ProductKey': 'ProductDesc',
                'AttributeKey': 'AttributeDesc',
                'AttributeValueKey': 'AttributeValueDesc'}
    ...
    raw_df_grp = raw_df[['ProductDesc', 'AttributeDesc', 'AttributeValueDesc'
                         ]].groupby(['ProductDesc', 'AttributeDesc'])
    ...
    skuCount = len(df)
    for col in df:
        if sum(df[col].notnull()) / skuCount < MIN_COMPLETION:
            del df[col]
            if VERBOSE:
